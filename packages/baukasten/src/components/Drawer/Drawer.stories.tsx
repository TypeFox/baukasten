...
const meta = {
    ...
    parameters: {
        docs: {
            description: {
                component:
                    'A slide-in panel component that enters from any edge of the viewport. Supports multiple sizes, placements, backdrop variants, and composable structure.',
            },
        },
    },
    tags: ['autodocs'],
    argTypes: {
        ...
        size: {
            control: 'text',
            description:
                'Size of the drawer (width for left/right, height for top/bottom). Accepts preset sizes (xs, sm, md, lg, xl, fullscreen) or any valid CSS length (e.g. "400px", "70vh", "50vw").',
            table: {
        ...
        <div style={{ display: 'flex', gap: '1rem', padding: '2rem', flexWrap: 'wrap' }}>
            {sizes.map((size) => (
                <React.Fragment key={size}>
                    <Button onClick={() => setOpenSize(size)}>{size.toUpperCase()} Drawer</Button>
                    <Drawer open={openSize === size} onClose={() => setOpenSize(null)} size={size}>
                        ...
                        <DrawerBody>
                            ...
                            <Paragraph>
                                Size controls the width for left/right drawers and the height for
                                top/bottom drawers.
                            </Paragraph>
                        </DrawerBody>
                        <DrawerFooter>
...
const FullscreenComponent = () => {
    ...
    return (
        <div style={{ padding: '2rem' }}>
            ...
            <Drawer open={isOpen} onClose={() => setIsOpen(false)} size="fullscreen">
                <DrawerHeader onClose={() => setIsOpen(false)}>Fullscreen Drawer</DrawerHeader>
                <DrawerBody>
                    <div
                        style={{
                            display: 'flex',
    ...
    return (
        <div style={{ display: 'flex', gap: '1rem', padding: '2rem', flexWrap: 'wrap' }}>
            {customSizes.map((size) => (
                <React.Fragment key={size}>
                    <Button onClick={() => setOpenSize(size)}>{size}</Button>
                    <Drawer open={openSize === size} onClose={() => setOpenSize(null)} size={size}>
                        <DrawerHeader onClose={() => setOpenSize(null)}>
                            Custom size: {size}
                        </DrawerHeader>
    ...
    return (
        <div style={{ padding: '2rem' }}>
            ...
            >
                ...
                <DrawerBody>
                    <Paragraph>
                        This drawer uses the <Text weight="bold">{variant || 'solid'}</Text>{' '}
                        backdrop variant.
                    </Paragraph>
            ...
            >
                ...
                <Paragraph>
                    Open the drawers above to see how different backdrop styles affect this content.
                </Paragraph>
...
const ScrollableContentComponent = () => {
    ...
    return (
        <div style={{ padding: '2rem' }}>
            ...
            <Drawer open={isOpen} onClose={() => setIsOpen(false)} size="md">
                <DrawerHeader onClose={() => setIsOpen(false)}>Activity Log</DrawerHeader>
                <DrawerBody>
    ...
    return (
        ...
        >
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {['top', 'right', 'bottom', 'left'].map((placement) => (
                        <Button
                            key={placement}
                            size="sm"
                            onClick={() => setActiveDemo(`placement-${placement}`)}
                        >
            ...
            <section style={{ marginBottom: '3rem' }}>
                ...
                <Paragraph color="muted">
                    Available sizes: xs (280px), sm (320px), md (400px), lg (560px), xl (720px),
                    fullscreen
                </Paragraph>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            <section style={{ marginBottom: '3rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <Button size="sm" onClick={() => setActiveDemo('backdrop-solid')}>
                        Solid
                    </Button>
                    <Button size="sm" onClick={() => setActiveDemo('backdrop-blur')}>
                        Blur
                    </Button>
                    <Button size="sm" onClick={() => setActiveDemo('backdrop-transparent')}>
                        Transparent
                    </Button>
                </div>
            </section>
            ...
            <section style={{ marginBottom: '3rem' }}>
                ...
                <Paragraph color="muted">Common drawer patterns for different use cases</Paragraph>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <Button size="sm" onClick={() => setActiveDemo('nav')}>
                        Navigation
                    </Button>
                    <Button size="sm" onClick={() => setActiveDemo('settings')}>
                        Settings
                    </Button>
            ...
            <Drawer
                ...
                size="sm"
            >
                <DrawerHeader onClose={() => setActiveDemo(null)}>Navigation</DrawerHeader>
                <DrawerBody>
                    <Menu>
                        <MenuItem icon={<Icon name="home" />} onClick={() => setActiveDemo(null)}>
                            Home
                        </MenuItem>
                        <MenuItem
                        ...
                        <MenuItem icon={<Icon name="file" />} onClick={() => setActiveDemo(null)}>
                            Documents
                        </MenuItem>
                        <MenuDivider />
                        <MenuItem
            ...
            {/* Settings drawer */}
            <Drawer open={activeDemo === 'settings'} onClose={() => setActiveDemo(null)} size="md">
                <DrawerHeader onClose={() => setActiveDemo(null)}>Settings</DrawerHeader>
                <DrawerBody>
            ...
            {/* Scrollable drawer */}
            <Drawer open={activeDemo === 'scroll'} onClose={() => setActiveDemo(null)} size="md">
                <DrawerHeader onClose={() => setActiveDemo(null)}>Scrollable Content</DrawerHeader>
                <DrawerBody>
