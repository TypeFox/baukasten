...
export function applyDataTransaction<TData>(
    ...
    if (tx.remove && tx.remove.length > 0) {
        ...
        if (removeIds.size > 0) {
            nextData = nextData.filter((r, i) => !removeIds.has(getRowId(r, i)));
        }
    }
...
export function useDataTableData<TData>(
    ...
    const applyTransaction = useCallback(
        (
            tx: DataTableTransaction<TData>,
            undoable?: boolean,
        ): DataTableTransactionResult<TData> | UndoableDataTableTransactionResult<TData> => {
            const currentData = dataRef.current;
            const snapshotBeforeTx = undoable ? [...currentData] : undefined;

            const { nextData, result } = applyDataTransaction(currentData, tx, getRowIdRef.current);
    ...
    const applyTransactionAsync = useCallback(
        (
            tx: DataTableTransaction<TData>,
            undoable = false,
        ): Promise<DataTableTransactionResult<TData>> => {
        ...
        return () => {
            ...
            if (queue.length > 0) {
                ...
                const error = new Error(
                    'Component unmounted before async transactions were flushed',
                );
