...
const ExternalHookExample = () => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--bk-spacing-4)' }}>
            ...
            <DataTable data={data} columns={personColumns} getRowId={(row) => row.id} bordered />
        </div>
    );
};
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <DataTable
                ...
                onAsyncTransactionsFlushed={(event) => {
                    addLog(
                        `onFlushed — add:${event.result.add.length} upd:${event.result.update.length} rem:${event.result.remove.length} undo:${event.undo ? 'yes' : 'no'}`,
                    );
                }}
            />
            <div
                style={{
                    fontFamily: 'monospace',
                    fontSize: 12,
                    background: 'var(--bk-color-bg-muted, #f5f5f5)',
                    padding: 8,
                    borderRadius: 4,
                    minHeight: 60,
                }}
...
const VirtualizedExample = () => {
    const largeData = useMemo(() => generateUsers(10_000), []);
    return (
...
export const Virtualized: Story = {
    ...
    parameters: {
        docs: {
            description: {
                story: `
Row virtualization is **enabled by default**. The table only renders the rows visible in the viewport (plus a configurable overscan), which enables smooth scrolling even with 10,000+ rows.

Virtualization requires a constrained scroll height — use \`fillHeight\` (recommended) or \`maxHeight\`.

- **\`estimatedRowHeight\`** — initial row height estimate in px (default: 35). Rows are measured dynamically after render.
- **\`overscan\`** — rows rendered outside the viewport (default: 5). Higher values reduce flicker on fast scroll.
- **\`disableRowVirtualization\`** — set to \`true\` to opt out and render all rows.

Virtualization is automatically disabled when pagination is enabled (paginated data is already small).
`,
            },
        },
    },
