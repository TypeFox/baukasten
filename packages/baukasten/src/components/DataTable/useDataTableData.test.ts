...
describe('applyDataTransaction (pure)', () => {
    ...
    it('warns when removing a row that does not exist', () => {
        ...
        expect(result.remove).toEqual([]);
        expect(result.warnings).toEqual([{ type: 'remove-not-found', row: ghost, id: '999' }]);
    });
    ...
    it('warns when updating a row that does not exist', () => {
        ...
        expect(nextData).toEqual(initialPeople);
        expect(result.update).toEqual([]);
        expect(result.warnings).toEqual([{ type: 'update-not-found', row: ghost, id: '999' }]);
    });
    ...
    it('warns when updating a row that was removed in the same transaction', () => {
        ...
        expect(result.remove).toEqual([bob]);
        expect(result.update).toEqual([]);
        expect(result.warnings).toEqual([{ type: 'update-not-found', row: updatedBob, id: '2' }]);
    });
    ...
    it('handles empty transaction gracefully', () => {
        const { nextData, result } = applyDataTransaction(initialPeople, {}, getRowId);
    ...
    it('handles empty data array', () => {
        const { nextData, result } = applyDataTransaction([], { add: [alice, bob] }, getRowId);
...
describe('useDataTableData', () => {
    ...
    it('defaults to empty array when initialData is omitted', () => {
        const { result } = renderHook(() => useDataTableData({ getRowId }));
    ...
    it('calls onDataChange after applyTransaction', () => {
        ...
        expect(onDataChange).toHaveBeenCalledWith([...initialPeople, diana], { add: [diana] });
    });
    ...
    it('works with sequential transactions correctly', () => {
        const { result } = renderHook(() => useDataTableData({ initialData: [alice], getRowId }));
...
describe('useDataTableData — applyTransactionAsync', () => {
    ...
    it('merges multiple async transactions into one batch', async () => {
        ...
        expect(r1).toBe(r2);
        expect(result.current.data).toEqual([alice, { id: '2', name: 'Bobby', age: 25 }, charlie]);
    });
