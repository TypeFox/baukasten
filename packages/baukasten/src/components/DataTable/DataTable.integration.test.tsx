...
// Disable row virtualization in tests — jsdom has no layout engine
const TABLE_TEST_PROPS = { disableRowVirtualization: true } as const;
...
describe('DataTable — managed mode (initialData + ref)', () => {
    ...
    it('ref.applyTransaction({ remove }) removes a row from the DOM', () => {
        const TestComponent = () => {
            ...
            return (
                <div>
                    <button onClick={() => ref.current?.applyTransaction({ remove: [alice] })}>
                        Remove Alice
                    </button>
                    <DataTable
                        {...TABLE_TEST_PROPS}
        ...
        const TestComponent = () => {
            ...
            return (
                <div>
                    <button
                        onClick={() =>
                            ref.current?.setData([{ id: '10', name: 'Xavier', role: 'CTO' }])
                        }
                    >
                        Replace data
                    </button>
        ...
        const TestComponent = () => {
            ...
            return (
                <>
                    ...
                    <button onClick={() => ref.current?.flushAsyncTransactions()}>Flush</button>
                </>
            );
        };
